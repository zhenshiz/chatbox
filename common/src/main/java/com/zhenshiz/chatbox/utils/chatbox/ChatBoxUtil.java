...
public class ChatBoxUtil {
    ...
    private static List<Portrait<?>> bakePortrait(Identifier newRl, String newGroup, Integer newIndex) {
        ...
        // 如果恰好是下一句对话，直接设置
        if (newRl.equals(dialoguesIdentifier) && newGroup.equals(group) && index != null && newIndex == index + 1) {
    ...
    public static void playVideo(String json) {
        ...
        if (!ChatBox.isWaterMediaLoaded()) {
            minecraft.player.sendSystemMessage(Component.literal("Failed to play video because Watermedia mod is not loaded!"));
            return;
        }
        try {
