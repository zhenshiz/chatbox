...
public class ChatBoxCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher, CommandBuildContext buildContext, Commands.CommandSelection commandSelection) {
        dispatcher.register(literal("chatbox").requires(Commands.hasPermission(Commands.LEVEL_GAMEMASTERS))
                .then(literal("theme")
                        .then(argument("Theme", IdentifierArgument.id()).suggests((context, builder) -> SharedSuggestionProvider.suggestResource(ChatBoxThemeLoader.themeMap.keySet(), builder))
                                .executes(ChatBoxCommand::toggleTheme)
                        )
                )
                .then(literal("skip")
                        .then(argument("Dialogues", IdentifierArgument.id()).suggests((context, builder) -> SharedSuggestionProvider.suggestResource(ChatBoxDialoguesLoader.dialoguesMap.keySet(), builder))
