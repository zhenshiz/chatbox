...
public interface EventExecutor {
     ...
     */
    static void registerEvent(String type, Consumer<String> executeOnClient, Supplier<Boolean> shouldExecuteOnServer, BiConsumer<ServerPlayer, String> executeOnServer) {
     ...
     * @param executeOnClient       组件事件触发时，在客户端执行的操作
     */
    static void registerClientEvent(String type, Consumer<String> executeOnClient) {
     ...
     */
    static boolean executeEvent(String type, String value) {
        EventExecutor executor = EXECUTORS.getOrDefault(type.toUpperCase(), null);
        if (executor != null) {
            executor.execute(value == null ? "" : value);
            return true;
        } else {
