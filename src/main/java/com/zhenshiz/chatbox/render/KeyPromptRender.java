...
public class KeyPromptRender extends AbstractComponent<KeyPromptRender> {
    ...
    public void render(GuiGraphics guiGraphics, float pPartialTick) {
        if (this.visible) {
            ...
            //right scroll
            RenderUtil.renderImage(guiGraphics, BeanUtil.getValueOrDefault(this.rightClickTexture, ResourceLocation.parse("chatbox:textures/key/right_mouse.png")), x, y + 2, 0, 12, 16, 1, opacity);
            drawText(guiGraphics, x + 14, y + (float) font.lineHeight / 2, keyRightClick);

            x += 18 + font.width(keyRightClick);

            //mouse scroll
            RenderUtil.renderImage(guiGraphics, BeanUtil.getValueOrDefault(this.scrollTexture, ResourceLocation.parse("chatbox:textures/key/scroll_mouse.png")), x, y + 2, 0, 12, 16, 1, opacity);
